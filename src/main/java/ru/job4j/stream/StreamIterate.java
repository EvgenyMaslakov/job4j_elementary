 ...
 * Т.е. так как мы это делаем с обычным for. Arrays.stream() вернет нам стрим элементов
 * массива, но что если нам нужен еще индекс?
 *
 * Есть метод Stream.iterate() который позволяет сгенерировать нам стрим, по заданным
 * условиям. Его мы можем использовать для нашей задачи, мы генерируем стрим индексов,
 ...
 * потому что i++ эквивалентно i += 1, а это предполагает изменение переменной
