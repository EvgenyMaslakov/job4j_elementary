 ...
 *
 * В интерфейсе Map<K, V> определен особый метод, который позволяет вычислить значение в том случае,
 ...
 * заданный ключ key еще не связан со значением (или значение равно null), будет выполнено вычисление
 * значение в соответствии с использованием заданной функции mappingFunction и это значение будет
 ...
 * map.computeIfAbsent(2, key -> "second" + "_" + key); // вычисляем значение для ключа, которого
 * нет в отображении
 * System.out.println(map.get(2)); // будет выведено second_2
 * map.computeIfAbsent(1, key -> "first" + "_" + key); // вычисляем значение для ключа, которого
 * нет в отображении
