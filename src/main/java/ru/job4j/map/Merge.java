 ...
 * связывание ключа key с переданным значением value, если указанный ключ еще не связан со значением, или
 * связан с нулевым значением. В противном случае произойдет замена значения, которое соответствует key,
 ...
 * map.merge(1, "Clinton", (oldV, newV) -> oldV + " " + newV); // заменяем значением новой строки, пары,
 * которая есть в отображении
 * System.out.println(map.get(1)); // будет выведено Bill Clinton
 * map.merge(2, "Trump", (oldV, newV) -> oldV + " " + newV); // если такого ключа нет в отображении - пара
 * будет просто создана
 * System.out.println(map.get(2)); // будет выведено Trump
 * Задание: метод принимает отображение - число(id пользователя) - строка(имя пользователя), а также список
 * пользователей. Возникла проблема - данные в отображении хранились так долго и ранее не было предусмотрено,
