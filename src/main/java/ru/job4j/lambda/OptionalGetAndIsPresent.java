 ...
 *
 * Запомните, перед вызовом метода get() всегда нужно использовать isPresent(). В этом суть Optional,
 * чтобы программист не забывал делать проверки.
 *
 * Ваша задача:
 *
 * 1. Написать метод indexOf(). Он ищет индекс по значению. Если индекс не найден, должно вернуться
 * Optional.empty(). Если найден, то Optional из этого индекса
 *
 * 2. В методе get() нужно вызвать метод indexOf(). Если вернулся пустой Optinal, то метод должен
