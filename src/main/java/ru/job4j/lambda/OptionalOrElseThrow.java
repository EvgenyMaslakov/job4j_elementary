...
/**
 * 1.4.1. Lambda
 * 5.5. Optional. Методы orElseThrow()
 *
 * Еще одной частой задачей бывает валидация данных, т.е. действия вида: если проверка прошла,
 * то выкинуть исключение, иначе продолжить работать дальше. Опять же относительно Optional мы могли
 * бы написать, если Optional пустой, то выкинуть исключение. На этот случай в Optional также есть
 * метод orElseThrow(), этот метод перегружен. Метод работает так, если значение существует, то оно
 * возвращается, иначе вылетает исключение. Метод orElseThrow() который ничего не принимает кидает
 * исключение NoSuchElementException, в перегруженную его версию мы можем отправить любое исключение
 *
 * Например
 *
