 ...
 *
 * Method reference (ссылка на метод) - это языковая конструкция, которая
 * представляет собой упрощенную
 * запись лямбда выражений.
 *
 * Когда можно применить ссылки на методы? Основное условие - это совпадение
 * сигнатур методов в функциональном
 * интерфейсе и вызываемом методе. Сигнатура метода - это имя метода и его
 * входные параметры.
 *
 * Например, есть интерфейс Consumer. Сигнатура метода accept() с возвращаемым
 * значением выглядит так
 * void accept(T).
 * Т.е. метод должен что-то принимать и ничего не возвращать. С ним можно
 * например применять метод
 * System.out.println()
 *
 ...
 * методы бывают статические и
 * не статические. К статическим методам можно получить доступ через имя класса,
 * как например, в последнем
 * примере. К не статическим методам можно получить доступ через объект класса,
 * например,
