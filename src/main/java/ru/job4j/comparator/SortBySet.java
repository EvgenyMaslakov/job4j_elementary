 ...
 * Второй очевидный недостаток, как нам сортировать,
 * например Set.
 *
 * По возможности, если нам не требуется далее работать с исходной коллекцией
 * и пользоваться ее преимуществами,
 * мы можем делать следующее.
 * Для сортировки сетов мы можем создать новый сет уже отсортированный
 * (в случае
 * если у нас есть неотсортированный сет, например LinkedHashSet, HashSet).
 * Новый сет будет TreeSet, который
