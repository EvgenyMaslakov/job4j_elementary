 ...
 *
 * Если мы хотим, что бы null элементы скопились в конце мы можем использовать компаратор, созданный с
 * помощью метода Comparator.nullsLast(). Он аналогичен nullsFirst() за исключением, что null элементы
 * при его применении скопятся в конце.
 *
 * Например:
 *
