 ...
 *
 * Существует одноименный метод Comparator.naturalOrder(), этот метод возвращает компаратор, основанный
 * на реализации метода compareTo() у типа, который его вызывает.
 *
 * Если вызвать этот метод для числа, например для int, произойдет autoboxing к Integer, и будет
 * использована реализация compareTo в классе Integer.
 *
